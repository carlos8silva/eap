...
public class UTUtil extends UnitTester {
	...
	public void testCreateObject() throws Throwable {
		...
		// Throw ConstructorNotVisibleException:
		try {
		...
		// Throw IllegalConstructionTargetException (abstract class):
		try {
		...
		// Throw IllegalConstructionTargetException (interface):
		try {
		...
		// Throw ConstructorInvocationException (interface):
		try {
			Tools.createObject(ToCreate.class, "");
			fail();
		}
	...
	public void t03_Wildchars() {
		// 1) Name matching
		String[] names = {
			"Carlos Silva", "Isabel Figueiredo", "João Madeira", "Darren King", "Quim Tenebroso",
			"Johanna Wilson", "Luís Soares", "Farrokh Bulsara", "Rebekah Staunton", "Zhu Malik"
		};
		_matchRegexp("*sa", names, new String[] {"Isabel Figueiredo", "Farrokh Bulsara"});
		_matchRegexp("*sa*", names, new String[] {"Isabel Figueiredo", "Farrokh Bulsara"});
		_matchRegexp("*", names, names);
		_matchRegexp("*So*", names, new String[] {"Quim Tenebroso", "Johanna Wilson", "Luís Soares"});
		_matchRegexp("Isa*", names, new String[] {"Isabel Figueiredo"});
		_matchRegexp("*ra", names, new String[] {"João Madeira", "Farrokh Bulsara"});
		// 2) Filename matching:
	...
	public void t04_CorrectGenExpr() {
		...
		// TODO ctx is not part of built ins now, need to move this to Config UT
		//assertEquals(Expr.evaluate("[ctx('tab-length')]"), "4");
		//assertEquals(Expr.evaluate("[ctx('/run-mode')]"), "PROD");
		//assertEquals(Expr.evaluate("[min('AbCDEfgH')]"), "abcdefgh");
		//assertEquals(Expr.evaluate("[ctx('abc')]"), "null");
	}
	...
	// TODO convert to unit test with assertions
	...
	public void testIdentifiers() {
		...
		assertFalse(Identifiers.isTextIdentifier("abc$"));
		// Code identifier to text identifier:
		...
		// Text identifier to code identifier:
